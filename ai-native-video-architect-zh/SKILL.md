...
用户要求全套时，`CREATE`或`TRANSFORM`完成剧本并通过剧本门禁后，先建立`NARRATIVE_LOCK`，再自动进入`ADAPT`。

任务路由必须保持创作中立：

- 只写故事、剧本或视觉脚本时，输出文本后停止，不强制生成完整视觉策略和资产；
- 只诊断剧本时，不进入视觉制作；
- 已有成立剧本并要求制作时，从`NARRATIVE_LOCK`之后开始；
- 单张生图或单镜头视频只建立局部视觉合同，不伪称锁定整部作品；
- 完整制作包才执行项目视觉策略、资产、Shot、CF和全部Prompt。
...
2. 在不套固定审美模板的前提下建立人物处境、不可见矛盾、主机制和结尾；
3. 完成可制作剧本或视觉脚本；
4. 通过剧本门禁并建立`NARRATIVE_LOCK`；
5. 根据当前剧本证据推导`PROJECT_VISUAL_STRATEGY`，只对本项目生效；
6. 预分析全部镜头需要的景别、正侧背角度、全身动作、手部交互、状态变化和空间方向；
7. 规划覆盖这些实际需求的角色、场景、道具与状态资产；
8. 为所有规划资产编写完整生图Prompt；
9. 拆解完整镜头表；
10. 为每个镜头建立导演意图、观众位置、信息优先级与揭示顺序；
11. 为每个镜头建立摄影方向、逐镜灯光、表演方向和情绪曲线；
12. 为每个镜头建立开始状态、主要动作和结束状态；
13. 选择控制帧与视频生成方式；
14. 为每个镜头编写生图Prompt或明确继承来源；
15. 为每个镜头编写完整生视频Prompt；
16. 对相邻镜头执行连续性、状态进程与情绪强度检查；
17. 对NARRATIVE_LOCK、项目视觉策略、资产角度/交互/状态覆盖、Shot、CF和Prompt执行完整检查；
18. 对高风险镜头执行可生成性降级；
19. 最多进行两轮内部返修；
20. 输出完整制作提示词包。
...
## 6. NARRATIVE_LOCK、项目视觉策略与规划资产

### NARRATIVE_LOCK

正式制作前锁定：主角或主体、人物关系、核心处境、世界规则或主要机制、关键选择、高潮行动者、不可逆变化、结尾与开放程度、主题意义及用户授权边界。

后续可以改变实现方式、机位、镜头数量和生成方法，但不得为了风格擅自改写这些叙事事实。

### 项目视觉策略只在制作阶段启用

读取`controllers/project-visual-strategy.md`。

只写剧本、只诊断或只改写文本时，不强制建立完整视觉策略。`ADAPT`和`FULL_CREATION_PACKAGE`在`NARRATIVE_LOCK`之后生成当前项目专属的`PROJECT_VISUAL_STRATEGY`。

该策略必须：

- 标记`scope: PROJECT_ONLY`；
- 从当前剧本、类型、观众体验、时长、画幅、平台和生成条件推导；
- 用户未指定成熟方向时，内部探索2—4个真正不同的视觉方向再选择；
- 定义Style DNA、人物、空间、道具、色彩、真实光源、材质、摄影、表演、声音、背景职责和视觉张力；
- 定义允许变化和禁止漂移；
- 不把冷灰、诗性、写实、慢镜头、浅景深、古典或任何审美写成Skill通用默认；
- 不得反向改变`NARRATIVE_LOCK`。

单张图片和单镜头视频只建立局部视觉合同，不伪称已经锁定整部作品。
...
资产不是越少越好，也不是所有项目固定做全套。先读取实际镜头会出现的景别、正侧背角度、身体动作、手部交互、服装结构、状态变化、场景方向和道具阶段，再决定资产集合。
...
- 精确手部叙事：手部与核心道具交互参考；
- 污染、伤损、湿水、变装等累积：状态进程参考；
- 正反打、反向机位或关键空间局部：对应环境参考；
- 道具页面、开合、破损、燃烧或阶段变化：道具结构与状态参考。

三视图不能代替重要面部近景，面部图也不能代替服装背面、发型后部和全身比例。

判断标准是：最终所有Shot是否都能明确引用足够的身份、角度、结构、空间、交互、道具与状态依据，同时没有只为显得专业而创建的冗余资产。
